...
- 테스트 클래스: `android/app/src/androidTest/.../autofill/AutofillE2ETest.kt`
  - 1단계 `noAuthFill`: 볼트 생성 → 계정 생성 → 자동완성 활성화 → sync → testHost fill 검증
  - 2단계 `authResync`: PIN 설정 → 재동기화(재래핑 auth-required) → 35초 Keystore 캐시 만료 대기 → testHost 진입 → auth dataset 탭 → BiometricPrompt PIN 인증 → 실제 계정 fill 검증
