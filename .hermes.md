...
npm run test:e2e:android                 # 전체 빌드+실행 (1단계 noAuth + 2단계 authRequired)
npm run test:e2e:android:fast            # APK 재사용 (빠른 반복)
powershell -File android/run-autofill-e2e.ps1 -TestMethod resyncAfterDeviceCredentialAdded_authRequired  # 단일 테스트
...
### Android Autofill E2E
- 실행: `npm run test:e2e:android` (빌드 포함) 또는 `npm run test:e2e:android:fast` (APK 재사용). 스크립트: `android/run-autofill-e2e.ps1`
- 사전 조건: Android 에뮬레이터 실행 중 (`adb devices`)
- 테스트 클래스: `android/app/src/androidTest/.../autofill/AutofillE2ETest.kt`
  - 1단계 `autofillEnableSyncAndFill_unencryptedVault_noAuth`: 볼트 생성 → 계정 생성 → 자동완성 활성화 → sync → testHost fill 검증
  - 2단계 `resyncAfterDeviceCredentialAdded_authRequired`: PIN 설정 → 재동기화(재래핑 auth-required) → 35초 Keystore 캐시 만료 대기 → testHost 진입 → auth dataset 탭 → BiometricPrompt PIN 인증 → 실제 계정 fill 검증
- 스크립트가 시작 시 PIN 설정/종료 시 PIN 해제를 자동 처리한다. `-SkipBuild`로 APK 재설치 생략 가능
- **debug 빌드 한정**: Keystore 마스터 키 인증 유효시간 30초(릴리스는 30분) — 인증 프롬프트 시나리오를 빠르게 재현하기 위함 (`KeystoreManager.generateNewKey`)
- 알려진 이슈: BiometricPrompt 창은 접근성 트리에 노출되지 않아 UIAutomator 감지 불가 → 테스트는 키코드 직접 입력(`inputPinViaKeyEvents`) 사용; auth dataset 항목은 이모지 접두사가 있어 부분 문자열 매칭(`clickByTextContains`) 사용
...
4. **Autofill testing** — `npm run test:e2e:android` (에뮬레이터 필요) 또는 실기기 수동 검증; 상세는 위 "Android Autofill E2E" 섹션
