...
export const editFilename = async (directoryUri, filepath, newFilename, pincode) => {
	const contents = await readFile(filepath, pincode);
	console.log(contents);
	console.log(directoryUri, filepath, newFilename, pincode);
	const newFilepath = await createFile(directoryUri, newFilename, JSON.stringify(contents), pincode);
	if (newFilepath) {
...
export const editPincode = async (filepath, pincode, newPincode) => {
	const contents = await readFile(filepath, pincode);
	const newFilepath = await modifyFile(filepath, JSON.stringify(contents), newPincode);
	return newFilepath;
};
export const createFile = (directoryUri, fileName, contents, pincode) => {
	console.log(directoryUri, fileName, contents, pincode);
	return StorageAccessFramework.createFileAsync(directoryUri, fileName, 'text/plain')
		.then((filepath) => {
