...
import { getDatabase, clearActiveFileInfo } from "./db";
...
describe("templateStorage Integration Tests", () => {
  ...
  describe("getAll", () => {
    it("sortOrder 순서로 모든 템플릿을 반환한다", async () => {
      // init() 없이 빈 DB에서 시작
      await templateStorage.create(createTemplateInput({ name: "C", sortOrder: 2 }));
      await templateStorage.create(createTemplateInput({ name: "A", sortOrder: 0 }));
      await templateStorage.create(createTemplateInput({ name: "B", sortOrder: 1 }));

      const result = await templateStorage.getAll();
  ...
  describe("getById", () => {
    it("ID로 템플릿을 조회한다", async () => {
      await templateStorage.init();
      const created = await templateStorage.create(createTemplateInput({ name: "조회 테스트" }));

      const result = await templateStorage.getById(created.id);
    ...
    it("존재하지 않으면 undefined를 반환한다", async () => {
      ...
      const result = await templateStorage.getById("nonexistent-id");
  ...
  describe("create", () => {
    it("UUID, 타임스탬프를 생성하여 템플릿을 저장한다", async () => {
      ...
      const result = await templateStorage.create(input);
    ...
    it("여러 템플릿 생성 시 각각 고유한 ID를 가진다", async () => {
      ...
      const t1 = await templateStorage.create(createTemplateInput({ name: "템플릿 1" }));
      const t2 = await templateStorage.create(createTemplateInput({ name: "템플릿 2" }));
      const t3 = await templateStorage.create(createTemplateInput({ name: "템플릿 3" }));
    ...
    it("필드 배열이 정확히 저장된다", async () => {
      ...
      const result = await templateStorage.create(createTemplateInput({ fields: customFields }));
  ...
  describe("update", () => {
    it("템플릿을 수정하고 updatedAt을 갱신한다", async () => {
      await templateStorage.init();
      const created = await templateStorage.create(createTemplateInput({ name: "원본" }));
      const originalUpdatedAt = created.updatedAt;

      // 약간의 시간 차이를 두기 위해 대기
      await new Promise((resolve) => setTimeout(resolve, 10));

      await templateStorage.update(created.id, { name: "수정된 이름", description: "새 설명" });
    ...
    it("존재하지 않는 ID로 업데이트해도 에러 없이 통과한다 (Dexie 특성)", async () => {
      ...
      await expect(
        templateStorage.update("nonexistent-id", { name: "테스트" }),
      ).resolves.not.toThrow();
    });

    it("필드만 부분 업데이트한다", async () => {
      await templateStorage.init();
      const created = await templateStorage.create(createTemplateInput());

      await templateStorage.update(created.id, {
        fields: [
  ...
  describe("delete", () => {
    it("템플릿을 삭제한다", async () => {
      // init() 없이 빈 DB에서 시작
      const created = await templateStorage.create(createTemplateInput({ name: "삭제할 템플릿" }));

      await templateStorage.delete(created.id);
    ...
    it("여러 템플릿 중 하나만 삭제한다", async () => {
      ...
      await templateStorage.delete(t2.id);

      const remaining = await templateStorage.getAll();
  ...
  describe("reorder", () => {
    it("트랜잭션 내에서 순서대로 sortOrder를 업데이트한다", async () => {
      ...
      const t1 = await templateStorage.create(createTemplateInput({ name: "첫 번째", sortOrder: 0 }));
      const t2 = await templateStorage.create(createTemplateInput({ name: "두 번째", sortOrder: 1 }));
      const t3 = await templateStorage.create(createTemplateInput({ name: "세 번째", sortOrder: 2 }));

      // 순서 변경: t3, t1, t2
      await templateStorage.reorder([t3.id, t1.id, t2.id]);
    ...
    it("빈 배열로 호출해도 에러 없이 통과한다", async () => {
      // init() 없이 빈 DB에서 시작
      await expect(templateStorage.reorder([])).resolves.not.toThrow();
    ...
    it("일부 ID만 전달해도 해당 ID들만 재정렬한다", async () => {
      // init() 없이 빈 DB에서 시작
      const t1 = await templateStorage.create(createTemplateInput({ name: "A", sortOrder: 0 }));
      const t2 = await templateStorage.create(createTemplateInput({ name: "B", sortOrder: 1 }));
      const t3 = await templateStorage.create(createTemplateInput({ name: "C", sortOrder: 2 }));

      // t1과 t3만 순서 바꾸기 (t2는 건드리지 않음)
      await templateStorage.reorder([t3.id, t1.id]);
  ...
  describe("통합 시나리오", () => {
    it("init -> create -> getAll -> update -> delete -> reorder 전체 흐름", async () => {
      ...
      await templateStorage.reorder(reorderedIds);

      all = await templateStorage.getAll();
      expect(all[0].id).toBe(userTemplate.id);
      expect(all[0].name).toBe("수정된 내 템플릿");

      // 6. 삭제
      await templateStorage.delete(userTemplate.id);
    ...
    it("내장 템플릿 사이에 사용자 템플릿을 끼워넣을 수 있다", async () => {
      ...
      const builtin = await templateStorage.getAll();
      ...
      const reordered = await templateStorage.getAll();
  ...
  describe("데이터 무결성", () => {
    it("createdAt은 생성 후 변경되지 않는다", async () => {
      ...
      await new Promise((resolve) => setTimeout(resolve, 10));
      await templateStorage.update(created.id, { name: "변경됨" });
    ...
    it("동시 생성 시 ID 충돌이 없다", async () => {
      ...
      const promises = Array.from({ length: 10 }, (_, i) =>
        templateStorage.create(createTemplateInput({ name: `동시 ${i}` })),
      );
    ...
    it("특수 문자가 포함된 이름/설명도 정상 저장된다", async () => {
      ...
      const created = await templateStorage.create(
