...
describe("templateTable - 템플릿 암호화 CRUD", () => {
  ...
  describe("restore / bulkRestore (백업/복원용)", () => {
    it("특정 ID로 템플릿을 복원한다 (평문)", async () => {
      const originalTemplate = createTestTemplates(1)[0];
      const templateToRestore: Template = {
        ...originalTemplate,
        id: "restored-42",
      ...
      const restored = await templateTable.restore(templateToRestore);

      expect(restored.id).toBe("restored-42");
      expect(restored.name).toBe("Restored Template");

      const retrieved = await templateTable.getById("restored-42");
      expect(retrieved).toBeDefined();
      expect(retrieved!.id).toBe("restored-42");
      expect(retrieved!.name).toBe("Restored Template");
      expect(retrieved!.createdAt).toBe(templateToRestore.createdAt);
    });

    it("특정 ID로 템플릿을 복원한다 (암호화)", async () => {
      const originalTemplate = createTestTemplates(1)[0];
      const templateToRestore: Template = {
        ...originalTemplate,
        id: "encrypted-99",
      ...
      await templateTable.restore(templateToRestore, testKey);

      const retrieved = await templateTable.getById("encrypted-99", testKey);
      expect(retrieved).toBeDefined();
      expect(retrieved!.id).toBe("encrypted-99");
      expect(retrieved!.name).toBe("Encrypted Restored");
      expect(retrieved!.fields).toEqual(originalTemplate.fields);
    });

    it("bulkRestore로 여러 템플릿을 한 번에 복원한다 (암호화)", async () => {
      const templates = createTestTemplates(3);
      const templatesToRestore = templates.map((t, i) => ({
        ...t,
        id: `bulk-${i + 1}`,
      ...
      await templateTable.bulkRestore(templatesToRestore, testKey);

      const all = await templateTable.getAll(testKey);
      expect(all).toHaveLength(3);

      // sortOrder 순으로 정렬되어야 함
      for (let i = 0; i < 3; i++) {
        const retrieved = await templateTable.getById(`bulk-${i + 1}`, testKey);
        expect(retrieved).toBeDefined();
        expect(retrieved!.id).toBe(`bulk-${i + 1}`);
        expect(retrieved!.name).toBe(`Bulk Template ${i + 1}`);
        expect(retrieved!.sortOrder).toBe(10 + i);
      }
    });
    ...
    it("bulkRestore 평문도 동작한다", async () => {
      ...
      await templateTable.bulkRestore(templatesToRestore);
